...
@[blueprint
  "divRemovable_zero"
  (statement := /-- Given a complex function $f$, we define the function
    $$g(z):=\begin{cases}
    \frac{f(z)}{z}, & z\neq 0;\\
    f'(0), & z=0.
    \end{cases}$$ -/)]
...
@[blueprint
  "divRemovable_zero_of_ne_zero"
  (statement := /-- Let $f$ be a complex function and let $z\neq 0$. Then, with $g$ defined as in
...
@[blueprint
  ...
  (statement := /-- Let $f$ be a complex function analytic on an open set $s$ containing $0$ such
    that $f(0)=0$.
    Then, with $g$ defined as in Definition~\ref{divRemovable_zero}, $g$ is analytic on $s$. -/)
  (proof := /-- We need to show that $g$ is complex differentiable at every point in $s$.
    ...
    \[
    \lim_{z\to 0}\frac{g(z)-g(0)}{z-0}=\lim_{z\to 0}\frac{\frac{f(z)}{z}-f'(0)}{z}=\lim_{z\to
    0}\frac{f(z)-f'(0)z}{z^2}=\lim_{z\to 0}\frac{f(z)-f(0)-f'(0)(z-0)}{(z-0)^2}=0,
    \]
...
@[blueprint
  ...
  (statement := /-- Let $f$ be a complex function analytic on the closed ball $|z|\leq R$ such that
    $f(0)=0$.
    Then, with $g$ defined as in Definition~\ref{divRemovable_zero}, $g$ is analytic on
    $|z|\leq R$. -/)
  (proof := /-- The proof is similar to that of Lemma~\ref{AnalyticOn_divRemovable_zero}, but we
    need to consider two cases:
    when $x$ is on the boundary of the closed ball and when it is in the interior.
    In the first case, we take a small open ball around $x$ that lies entirely within the closed
    ball,
    ...
    In both cases, we use the fact that $f(0)=0$ to ensure that the removable singularity at $0$ is
    handled correctly. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "schwartzQuotient"
  (statement := /-- Given a complex function $f$ and a real number $M$, we define the function
    $$f_{M}(z):=\frac{g(z)}{2M - f(z)},$$
    where $g$ is defined as in Definition~\ref{divRemovable_zero}. -/)]
...
@[blueprint
  ...
  (statement := /-- Let $M>0$. Let $f$ be analytic on the closed ball $|z|\leq R$ such that $f(0)=0$
    and suppose that $2M - f(z)\neq 0$ for all $|z|\leq R$.
    Then, with $f_{M}$ defined as in Definition~\ref{schwartzQuotient}, $f_{M}$ is analytic on
    $|z|\leq R$. -/)
  (proof := /-- This follows directly from Lemma~\ref{AnalyticOn_divRemovable_zero_closedBall} and
    the fact that the difference of two analytic functions is analytic. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  ...
  (statement := /-- Let $M>0$ and let $x$ be a complex number such that $\Re x\leq M$.
    Then, $|x|\leq|2M - x|$. -/)
...
@[blueprint
  ...
  (statement := /-- Let $R,\,M>0$. Let $f$ be analytic on $|z|\leq R$ such that $f(0)=0$ and suppose
    $\Re f(z)\leq M$ for all $|z|\leq R$. Then for any $0 < r < R$,
    $$\sup_{|z|\leq r}|f(z)|\leq\frac{2Mr}{R-r}.$$ -/)
  (proof := /-- Let
